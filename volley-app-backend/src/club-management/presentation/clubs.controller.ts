...
import { ListMembersQuery } from '../application/queries/list-members/list-members.query';
import { MemberListReadModel } from '../application/read-models/member-list.read-model';
...
export class ClubsController {
  ...
  @ApiOperation({
    ...
    description:
      "Récupère la liste des membres d'un club avec filtrage optionnel par rôle (OWNER, PLAYER, COACH).",
  })
  @ApiParam({
    name: 'id',
    description: 'ID unique du club',
  ...
  @ApiQuery({
    name: 'role',
    required: false,
    description: 'Filtrer par rôle (OWNER, PLAYER, COACH)',
    example: 'PLAYER',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description:
      'Non autorisé - Vous ne pouvez accéder qu aux membres de votre club',
  })
  async listMembers(
