...
export class Member {
  private constructor(
    ...
    public readonly invitedBy: string | null, // userId of the person who invited (null for OWNER)
  ...
  canManageClubSettings(): boolean {
    return this.role === ClubRole.OWNER;
  }
  ...
  canManageTeams(): boolean {
    return this.role === ClubRole.OWNER || this.role === ClubRole.COACH;
  }
  ...
  validateCanBeRemovedBy(removerRole: ClubRole): void {
    ...
    // Only OWNER or COACH can remove members
    if (removerRole !== ClubRole.OWNER && removerRole !== ClubRole.COACH) {
      throw new MemberInsufficientPermissionsException();
    }

    // OWNER cannot be removed by COACH (only by OWNER for transfer)
    if (this.role === ClubRole.OWNER && removerRole !== ClubRole.OWNER) {
      throw new MemberInsufficientPermissionsException(
