...
export class Club {
  ...
  /**
   * Check if a user is the owner or a coach of the club
   * Note: This checks only the club ownerId, not the member role.
   * For member role checks, use Member entity methods.
   */
  isOwnerOrCoach(userId: string, userClubRole?: string): boolean {
    // If user is the owner of the club (by ownerId), they have full access
    if (this.ownerId === userId) {
      return true;
    }

    // If user has COACH role in the club, they also have access
    // Note: userClubRole would need to be passed from the member context
    return userClubRole === 'COACH';
  }
