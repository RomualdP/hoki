...
export class AuthService {
   ...
   */
  async signupCoachViaInvitation(dto: SignupWithInvitationDto): Promise<{
    user: UserWithoutPassword;
    access_token: string;
    clubId: string;
    hadPreviousClub: boolean;
  }> {
    return this.signupWithInvitation(dto, 'COACH');
  }

  /**
   * Common logic for player/coach signup
   */
  private async signupWithInvitation(
    dto: SignupWithInvitationDto,
    expectedType: 'PLAYER' | 'COACH',
  ): Promise<{
    user: UserWithoutPassword;
    access_token: string;
    clubId: string;
