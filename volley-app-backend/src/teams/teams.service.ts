...
export class TeamsService {
  ...
  async findAll(query: QueryTeamsDto, userId: string) {
    ...
    if (!user || !user.clubId) {
      throw new ForbiddenException(
        'You must be part of a club to retrieve teams',
      );
    }
  ...
  async findOne(id: string, userId: string) {
    ...
    if (team.clubId !== user.clubId) {
      throw new ForbiddenException('You cannot access teams from another club');
    }
  ...
  async create(createTeamDto: CreateTeamDto, userId: string) {
    ...
    if (user.clubRole !== 'OWNER' && user.clubRole !== 'COACH') {
      throw new ForbiddenException('Only owners and coaches can create teams');
    }
